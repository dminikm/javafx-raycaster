...
public class Gameloop extends AnimationTimer {
  public Gameloop(Canvas cv) {
    ...
    this.world =
        LevelLoader.loadLevel("data/levels/level01.json", textureRegistry);
    this.renderer =
        new Renderer(canvasWidth, canvasHeight, this.world, textureRegistry);

    try {
      this.endImage =
          new Image(new File("data/assets/end_screen.png").toURI().toString());
    } catch (Exception e) {
    }
  }
  ...
  public void handle(long currentNanoTime) {
    if (!this.world.shouldEnd()) {
      ...
      if (this.world.getPlayer().health <= 0) {
        this.world.resetTo(
            LevelLoader.loadLevel(this.world.getName(), this.textureRegistry));
      }
  ...
  private void render(double delta) {
    ...
    pw.setPixels(0, 0, this.canvasWidth, this.canvasHeight,
        buffer.getPixelFormat(), buffer.getData(), 0, buffer.getWidth());
    ...
    gc.fillText("" + (int) this.world.getPlayer().health, 125,
        this.canvasHeight - 50);

    int ammo = this.world.getPlayer().getCurrentWeapon().getAmmo();
    gc.fillText("" + ((ammo < 0) ? "Inf" : ammo), this.canvasWidth - 125,
        this.canvasHeight - 50);
  ...
  private void renderEndScreen() {
    this.gc.drawImage(this.endImage, 0, 0, this.endImage.getWidth(),
        this.endImage.getHeight(), 0, 0, this.canvasWidth, this.canvasHeight);
  }
