...
public class Line implements RayCastable {
  ...
  public RaycastResult castRay(Vec2 start, Vec2 dir) {
    ...
    if (t1 >= 0.0 && (t2 >= 0.0 && t2 <= 1.0)) {
      ...
      res.startOffset = res.precisePosition.distance(this.start)
          / this.start.distance(this.end);//res.precisePositition.sub(this.start).len() / this.start.sub(this.end).len();
