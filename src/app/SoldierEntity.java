...
public class SoldierEntity extends MonsterEntity {
  SoldierEntity(Vec2 pos, Vec2 dir, List<AnimatedSprite> sprites,
  ...
  public void update(double delta, World world) {
    ...
    if (this.alerted) {
      ...
      EntityRaycastResult res =
          world.castRayEntity(this.position, playerDir.normalize(), this);
      if (playerDir.len() < 8
          && this.elapsedTime - this.attackDelay > this.lastAttack && res.hit
          && res.entity instanceof Player) {
    ...
    if (this.isFiring()) {
      ...
      if (this.sprites.get(this.currentSpriteIndex).getCurrentIndex() == 1
          && !this.attackSound.isPlaying()) {
        this.fire(world);
  ...
  private void fire(World world) {
    ...
    double angle = dir.toAngle()
        + (r.nextBoolean() ? 1 : -1) * (r.nextDouble() * this.spread);
