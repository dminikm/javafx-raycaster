...
public class World implements RayCastable {
  public World(String name, int[][] worldMap, Player p,
    ...
    this.toPlayerPaths =
        new Vec2[this.worldMap.length][this.worldMap[0].length];
  ...
  public RaycastResult castRay(Vec2 start, Vec2 dir) {
    ...
    while (!hit) {
      ...
      if (this.worldMap[mapY][mapX] != 0) {
        if (this.worldMap[mapY][mapX] == -1) {
          ...
          TileEntityRaycastResult res =
              (TileEntityRaycastResult) tent.castRay(pos, dir);
          res.distance += dist;
    ...
    if (this.worldMap[mapY][mapX] != -1) {
      ...
      r.side = side;
      r.precisePosition = new Vec2(start.x + perpWallDist * dir.x,
          start.y + perpWallDist * dir.y);
    ...
    for (RaycastResult res : results) {
      if (res.distance < smallest.distance
          && !(res instanceof EntityRaycastResult)) {
        smallest = res;
      }
    }
  ...
  public EntityRaycastResult castRayEntity(Vec2 start, Vec2 dir,
      Entity ignore) {
    ...
    if (this.worldMap[(int) pos.y][(int) pos.x] == -1) {
      ...
      for (Sprite sprite : sprites) {
        if ((int) sprite.pos.x == (int) pos.x
            && (int) sprite.pos.y == (int) pos.y && sprite.solid) {
          return false;
        }
      }
    } else {
  ...
  public boolean isFree(Vec2 pos, boolean ignoreEntities) {
    if (this.worldMap[(int) pos.y][(int) pos.x] == -1) {
      ...
      List<Sprite> sprites =
          (ignoreEntities) ? this.sprites : this.getAllSprites();
    ...
    for (Entity ent : this.entities) {
      ...
      if (vel.x > 0
          && (!this.isFree(new Vec2((int) pos.x + 1, (int) (pos.y - bb.h / 2)))
              || !this.isFree(
                  new Vec2((int) pos.x + 1, (int) (pos.y + bb.h / 2))))) {
      ...
      if (vel.y > 0
          && (!this.isFree(new Vec2((int) (pos.x - bb.w / 2), (int) pos.y + 1))
              || !this.isFree(
                  new Vec2((int) (pos.x + bb.w / 2), (int) pos.y + 1)))) {
  ...
  public List<Sprite> getAllSprites() {
    ...
    List<Sprite> entitySprites =
        this.entities.stream().filter((final Entity entity) -> {
  ...
  public TileEntity getTileEntityAt(int x, int y) {
    for (TileEntity tent : this.tileEntities) {
      ...
      if ((int) pos.x == x && (int) pos.y == y) return tent;
    }
  ...
  private boolean isInBounds(Vec2 pos) {
    return pos.x >= 0 && pos.y >= 0 && pos.y < this.worldMap.length
  ...
  public List<Vec2> getPathToPlayer(Vec2 pos) {
    ...
    if (this.toPlayerPaths[(int) pos.y][(int) pos.x] != null) { // Can get to player
      ...
      while (!currentPos.eqi(this.getPlayer().getPosition())) { // We havent reached the player
        Vec2 cameFrom =
            this.toPlayerPaths[(int) currentPos.y][(int) currentPos.x]; // Where this cell came from
