...
public class LevelLoader {
  public static World loadLevel(String fileName,
    ...
    World w = new World(fileName, parseLevel(json), p, parseTileEntities(json),
        parseEntities(json), parseSprites(json));
  ...
  private static List<Sprite> parseSprites(JSONObject json) {
    List<JSONObject> sprites =
        JSONUtils.getFromComplexPath(json, "map.sprites");

    return sprites.stream().map((final JSONObject sprite) -> {
      return new Sprite(JSONUtils.vecFromJson(sprite, "position"),
  ...
  private static Weapon parseWeapon(JSONObject json) {
    ...
    double fireDelay =
        ((Number) JSONUtils.getFromComplexPath(json, "fireDelay"))
            .doubleValue();
    int numShots =
        ((Number) JSONUtils.getFromComplexPath(json, "numShots")).intValue();
    double spread =
        ((Number) JSONUtils.getFromComplexPath(json, "spread")).doubleValue();
    int damage =
        ((Number) JSONUtils.getFromComplexPath(json, "damage")).intValue();
    double range =
        ((Number) JSONUtils.getFromComplexPath(json, "range")).doubleValue();
    AnimatedSprite animation =
        JSONUtils.getAnimatedSpriteFromJson(json, "animation.");
    AudioClip sound = JSONUtils.getAudioClipFromJson(json, "sound");

    return new Weapon(name, sound, animation, ammo, fireDelay, numShots, spread,
        damage, range, available);
  }

  private static List<Weapon> parseWeapons(JSONObject json) {
    List<Weapon> weapons = new ArrayList<Weapon>(0);
    List<JSONObject> jsonWeapons =
        JSONUtils.getFromComplexPath(json, "weapons");
  ...
  private static Player parsePlayer(JSONObject json) {
    ...
    List<Weapon> weapons =
        parseWeapons(JSONUtils.getFromComplexPath(json, "player"));
    AudioClip hurtSound =
        JSONUtils.getAudioClipFromJson(json, "player.hurtSound");
  ...
  private static List<Entity> parseEntities(JSONObject json) {
    List<JSONObject> entities =
        JSONUtils.getFromComplexPath(json, "map.entities");
  ...
  private static List<TileEntity> parseTileEntities(JSONObject json) {
    List<JSONObject> tileEntities =
        JSONUtils.getFromComplexPath(json, "map.tileEntities");
  ...
  private static TileEntity parseDoorTileEntity(JSONObject json) {
    ...
    int textureId =
        ((Number) JSONUtils.getFromComplexPath(json, "textureId")).intValue();
    AudioClip sound = JSONUtils.getAudioClipFromJson(json, "sound");
  ...
  private static Entity parseTurretEntity(JSONObject json) {
    Vec2 position = JSONUtils.vecFromJson(json, "position");
    AnimatedSprite animation =
        JSONUtils.getAnimatedSpriteFromJson(json, "sprite.");
    AnimatedSprite deadSprite =
        JSONUtils.getAnimatedSpriteFromJson(json, "deadSprite.");
    ...
    return new TurretEntity(position, animation, deadSprite, hurtSound,
        firingSound);
  }
  ...
  private static Entity parseDogEntity(JSONObject json) {
    ...
    List<AnimatedSprite> sprites =
        JSONUtils.getAnimatedSpritesFromJson(json, "sprites");
  ...
  private static Entity parseSoldierEntity(JSONObject json) {
    ...
    return new SoldierEntity(position, direction, sprites, hurtSound,
        attackSound);
  }
  ...
  private static Entity parseAmmoPickupEntity(JSONObject json) {
    ...
    int textureId =
        ((Number) JSONUtils.getFromComplexPath(json, "textureId")).intValue();
    int amount =
        ((Number) JSONUtils.getFromComplexPath(json, "amount")).intValue();
    ...
    return new AmmoPickupEntity(position, textureId, weaponName, amount,
        makeAvailable);
  }
