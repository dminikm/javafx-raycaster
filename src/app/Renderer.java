...
public class Renderer {
  ...
  public void renderWorld(double delta) {
    ...
    for (int x = 0; x < buffer.getWidth(); x++) {
      ...
      if (res.hit) {
        ...
        int drawEnd = Math.min(this.internalHeight - 1,
            lineHeigth / 2 + this.internalHeight / 2);

        Texture t = null;

        boolean applyShading = true;
        if (res instanceof TileEntityRaycastResult) {
          t = this.textureRegistry.getTextureForId(
              ((TileEntityRaycastResult) res).entity.getTextureId());
          applyShading =
              ((TileEntityRaycastResult) res).entity.canApplyShading();
        }

        if (res instanceof BlockRaycastResult) {
          t = this.textureRegistry
              .getTextureForId(((BlockRaycastResult) res).blockId);
        }
        ...
        for (int y = drawStart; y < drawEnd; y++) {
          int d =
              (int) (y * 256 - this.internalHeight * 128 + lineHeigth * 128);
          int texY = ((d * t.height) / lineHeigth) / 256;
          int c = t.getPixel(texX, texY);

          if (res.side == 1 && applyShading)
            c = 0xFF000000 | (((c & 0xFFFFFF) >>> 1) & 8355711);
  ...
  private void renderSprites(double delta) {
    ...
    for (Sprite sprite : sprites) {
      ...
      int spriteScreenX =
          (int) ((this.internalWidth / 2) * (1 + transformX / transformY));
      ...
      int drawEndY = Math.min(this.internalHeight - 1,
          spriteHeight / 2 + this.internalHeight / 2);

      int spriteWidth = Math.abs((int) (this.internalHeight / transformY));
      int drawStartX = Math.max(0, -spriteWidth / 2 + spriteScreenX);
      int drawEndX =
          Math.min(this.internalWidth - 1, spriteWidth / 2 + spriteScreenX);
