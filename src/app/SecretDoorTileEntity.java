...
public class SecretDoorTileEntity extends TileEntity {
    ...
    public void update(double delta, World world) {
        if (this.opening) {
            this.openingTime = Math.min(((double)this.path.size() - 1) - 0.001, this.openingTime + delta);

            int pathNum = (int)(openingTime);
            double pathOffset = this.openingTime - pathNum;


            Vec2 dir = this.path.get(pathNum + 1).sub(this.path.get(pathNum)).normalize();
    ...
    public RaycastResult castRay(Vec2 start, Vec2 dir) {
        ...
        TileEntityRaycastResult res = new TileEntityRaycastResult(new Rect(this.openingPosition.x - 0.5, this.openingPosition.y - 0.5, 1, 1).castRay(start.add(dir.mul(-0.001)), dir));
        res.entity = this;
