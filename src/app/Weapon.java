...
class Weapon {
  public Weapon(String name, AudioClip sound, AnimatedSprite animation,
      int ammo, double delay, int numShots, double spread, int damage,
      double range, boolean available) {
    this.name = name;
    this.sound = sound;
  ...
  public void fire(Vec2 pos, Vec2 dir, World world) {
    if (this.ammo == 0 || this.isFiring()) return;
    ...
    for (int i = 0; i < this.numShots; i++) {
      double angle = startAngle + // Angle \|/ (the first one)
      ...
      EntityRaycastResult res =
          world.castRayEntity(pos, Vec2.fromAngle(angle), world.getPlayer());
