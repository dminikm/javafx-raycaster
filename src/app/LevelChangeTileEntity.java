...
public class LevelChangeTileEntity extends TileEntity {
  ...
  public void update(double delta, World world) {
    if (this.change) {
      world.resetTo(
          LevelLoader.loadLevel(this.nextLevel, TextureRegistry.getInstance()));
    }
  }
  ...
  public RaycastResult castRay(Vec2 start, Vec2 dir) {
    TileEntityRaycastResult res = new TileEntityRaycastResult(
        new Rect((int) this.position.x, (int) this.position.y, 1, 1)
            .castRay(start.add(dir.mul(-0.001)), dir));
    res.entity = this;
    res.distance = 0;
